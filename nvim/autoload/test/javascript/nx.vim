...
let l:path = split(a:position['file'], '/')[0:-2]
  while len(path) > 0
    if filereadable(join(path, '/')."/project.json")
      let l:workspaces = readfile(join(path, '/')."/project.json")
      if exists('*json_decode')
        let l:name = json_decode(join(workspaces, ''))['name']
        let project = name
        break
      endif
    endif
  ...
  elseif a:type ==# 'file'
    ...
    return [project] " This will run the test for the project
  endif
endfunction
